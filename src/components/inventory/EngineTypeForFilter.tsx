...
const EngineTypeForFilter: React.FC = () => {
  // All Makes
  const EngineTypes: IQuery[] = [
    ...
    >
      ...
      <div className="py-3 px-2  ">
        ...
        <div>
          ...
          >
            {EngineTypes.map((EngineTypes: IQuery) => (
              <div
                key={EngineTypes.name}
                className={`flex items-center justify-between py-2 px-2 rounded ${
                  selectedQuery.includes(EngineTypes.name) ? "bg-blue-100" : ""
                }`}
              >
                <div className="flex items-center gap-2">
                  <input
                    type="checkbox"
                    id={`checkbox-${EngineTypes.name}`}
                    checked={selectedQuery.includes(EngineTypes.name)}
                    onChange={() => handleCheckboxChange(EngineTypes.name)}
                    className="mt-1 focus:ring-white focus:border-blue-500 h-4 w-4"
                  />
                  <label
                    htmlFor={`checkbox-${EngineTypes.name}`}
                    className={`${
                      selectedQuery.includes(EngineTypes.name)
                        ? "text-blue-600"
                        : "text-gray-900"
                    }`}
                  >
          ...
          {!isShowAllQuery && (
            <div className="mt-3">
              <button
                onClick={() => setShowAllQuery(true)}
                className="flex items-center gap-2 text-blue-600 hover:underline"
              >
                <IoMdArrowDropdown /> Show all Types A-Z
              </button>
            </div>
          )}
        </div>
...
export default EngineTypeForFilter;
