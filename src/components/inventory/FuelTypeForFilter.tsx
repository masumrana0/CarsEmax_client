...
const FuelTypeForFilter: React.FC = () => {
  // All Makes
  const FuelTypes: IQuery[] = [
    ...
    >
      ...
      <div className="py-3 px-2 ">
        ...
        <div className="mt-1">
          ...
          >
            {FuelTypes.map((fuealtype: IQuery) => (
              <div
                key={fuealtype.name}
                className={`flex items-center justify-between py-2   rounded ${
                  selectedQuery.includes(fuealtype.name) ? "bg-blue-100" : ""
                }`}
              >
                <div className="flex items-center gap-2">
                  <input
                    type="checkbox"
                    id={`checkbox-${fuealtype.name}`}
                    checked={selectedQuery.includes(fuealtype.name)}
                    onChange={() => handleCheckboxChange(fuealtype.name)}
                    className="mt-1 focus:ring-white focus:border-blue-500 h-4 w-4"
                  />
                  <label
                    htmlFor={`checkbox-${fuealtype.name}`}
                    className={`${
                      selectedQuery.includes(fuealtype.name)
                        ? "text-blue-600"
                        : "text-gray-900"
                    }`}
                  >
...
export default FuelTypeForFilter;
