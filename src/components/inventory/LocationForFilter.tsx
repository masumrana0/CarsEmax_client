...
const LocationForFilter: React.FC = () => {
  // All Makes
  const locations: IQuery[] = [
    ...
    >
      ...
      <div className="py-3 px-2  ">
        ...
        <div className="mt-1">
          ...
          >
            {locations.map((location: IQuery) => (
              <div
                key={location.name}
                className={`flex items-center justify-between py-2   rounded ${
                  selectedQuery.includes(location.name) ? "bg-blue-100" : ""
                }`}
              >
                <div className="flex items-center gap-2">
                  <input
                    type="checkbox"
                    id={`checkbox-${location.name}`}
                    checked={selectedQuery.includes(location.name)}
                    onChange={() => handleCheckboxChange(location.name)}
                    className="mt-1 focus:ring-white focus:border-blue-500 h-4 w-4"
                  />
                  <label
                    htmlFor={`checkbox-${location.name}`}
                    className={`${
                      selectedQuery.includes(location.name)
                        ? "text-blue-600"
                        : "text-gray-900"
                    }`}
                  >
          ...
          {!isShowAllState && (
            <div>
              ...
              >
                <IoMdArrowDropdown /> Show all states/provinces
              </button>
            </div>
          )}
        </div>
...
export default LocationForFilter;
