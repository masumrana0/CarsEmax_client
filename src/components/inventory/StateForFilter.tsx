    ...
    >
      ...
      <div className="py-3 px-2  ">
        ...
        <div className="mt-1">
          ...
          >
            {allStates.map((state: IState) => (
              <div
                key={state.name}
                className={`flex items-center justify-between py-2   rounded ${
                  selectedQuery.includes(state.name) ? "bg-blue-100" : ""
                }`}
              >
                <div className="flex items-center gap-2">
                  <input
                    type="checkbox"
                    id={`checkbox-${state.name}`}
                    checked={selectedQuery.includes(state.name)}
                    onChange={() => handleCheckboxChange(state.name)}
                    className="mt-1 focus:ring-white focus:border-blue-500 h-4 w-4"
                  />
                  <label
                    htmlFor={`checkbox-${state.name}`}
                    className={`${
                      selectedQuery.includes(state.name)
                        ? "text-blue-600"
                        : "text-gray-900"
                    }`}
                  >
